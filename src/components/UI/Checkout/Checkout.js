...
const Checkout = props => {
    ...
    totalQuantity, tableId, onOrderPost, loading, onRemoteItem, onUpdateItem, orderType,
    firstName, lastName, email, phoneNumber, address, restaurant } = props;
    ...
    const placeOrder = () => {
        let order = createJsonData(['table', 'total_price', 'details', 'notes', 'type'], 
            [tableId, totalPrice, [
                cartItems.map(cartItem => [cartItem.id, cartItem.quantity])
            ], orderNotes, orderType]);
        if (orderType === 'delivery') {
            order = createJsonData(['restaurant', 'total_price', 'details', 'notes', 'type', 'first_name', 'last_name',
                'email', 'phone_number', 'address'], [restaurant, totalPrice, [
                cartItems.map(cartItem => [cartItem.id, cartItem.quantity])
            ], orderNotes, orderType, firstName, lastName, email, phoneNumber, address]);
        }
        onOrderPost(order);
    	...
    	<div className="modal-dialog" role="document">
	        <div className="modal-content">
	            <div className="modal-header modal-header-lg dark bg-dark">
	                <div className="bg-image"><img src="/static/assets/img/photos/modal-add.jpg" alt="" /></div>
